	...
	const set: BaileysEventHandler<"messaging-history.set"> = async ({ contacts }) => {
		try {
			...
			emitEvent(
				...
				undefined,
				"error",
				`An error occured during contacts set: ${e instanceof Error ? e.message : e}`,
			);
		}
	};
	...
	const upsert: BaileysEventHandler<"contacts.upsert"> = async (contacts) => {
		try {
			...
			emitEvent(
				...
				undefined,
				"error",
				`An unexpected error occurred during contacts upsert: ${error instanceof Error ? error.message : error}`,
			);
		}
	};
		...
		for (const update of updates) {
			try {
				...
				emitEvent(
					...
					undefined,
					"error",
					`An error occured during contact update: ${e instanceof Error ? e.message : e}`,
				);
			}
		}
	};
